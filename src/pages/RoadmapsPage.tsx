...
function RoadmapTimelineItem({
  ...
  return (
    <article
      ref={stepRef}
      className={`relative scroll-mt-28 rounded-lg transition-[background-color,box-shadow] duration-500 ${
        isHighlighted
    ...
    >
      ...
      {!isLast ? (
        <span className="absolute top-5 left-[5px] bottom-0 w-px bg-border/80" />
      ) : null}
  ...
  const variantIconIds = useMemo(
    () =>
      Array.from(
        new Set(
          (roadmap?.ranges ?? [])
            .map((range) => range.variant.icon_id)
            .filter(
